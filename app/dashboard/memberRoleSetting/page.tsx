...
export default function MemberAccessPage() {
  ...
  // ✅ toggle permission (แก้ hydration)
  const togglePermission = (gi: number, pi: number) => {
    const updated = [...permissionGroups];
    updated[gi].permissions[pi].enabled = !updated[gi].permissions[pi].enabled;
              ...
              <div key={group.category} className="space-y-6">
                ...
                <div className="space-y-2">
                  {group.permissions.map((perm, pi) => (
                    <label
                      key={perm.id}
                      className="flex items-center justify-between p-4 rounded-2xl bg-slate-800/20 border border-slate-800/50 hover:border-blue-500/50 transition-all cursor-pointer group"
                    >
                      <span className="text-[11px] font-bold text-slate-400 group-hover:text-white uppercase tracking-wide">
                        {perm.label}
                      </span>

                      {/* ✅ FIX hydration แต่ยัง style เดิม */}
                      <div className="relative inline-flex items-center">
                        ...
                        />
                        <div className="w-9 h-5 bg-slate-700 rounded-full peer-checked:bg-blue-600 after:content-[''] after:absolute after:top-[4px] after:left-[4px] after:bg-slate-500 after:rounded-full after:h-3 after:w-3 after:transition-all peer-checked:after:translate-x-full peer-checked:after:bg-white"></div>
                      </div>
